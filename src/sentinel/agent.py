        ...
        while self.state.step_count < max_steps and not self.state.task_complete:
            ...
            try:
                ...
                if 'APPLY_CLICKED_LINKEDIN' in result:
                    ...
                    while True:
                        ...
                        if 'LINKEDIN_SUCCESS' in next_result:
                            ...
                            print(f"🎉 LinkedIn Application {self.linkedin_applications}/6 Submitted!")
                            submit_attempt_count = 0  # Reset on success
                            
                            if self.linkedin_applications >= 6:
                                print("✅ LinkedIn target (6 applications) reached. Task complete!")
                                self.state.task_complete = True
                                break
                ...
                if 'CHECKBOX_CLICKED' in result and 'NEED_MORE' not in result and 'naukri.com' in current_url:
                    ...
                    apply_result = await self._page.evaluate("""async () => {
                        ...
                        if (applyBtn && applyBtn.offsetParent !== null) {
                            ...
                            // MONITOR: Check for error snackbar for up to 2 seconds
                            for (let i = 0; i < 4; i++) {
                                await new Promise(r => setTimeout(r, 500));
    ...
    async def _handle_scripted_fallback(self) -> str:
        ...
        try:
            ...
            js_code = f"""async () => {{
                ...
                // Smart option matcher for select/radio/checkbox
                const findBestOptionMatch = (answer, options) => {{
                    ...
                    for (const option of options) {{
                        const optText = (option.text || option.label || option.value || option).toLowerCase();
                        ...
                        // Contains answer
                        else if (optText.includes(answerLower)) {{
                            score = 80;
                        }}
                        // Answer contains option
                        else if (answerLower.includes(optText) && optText.length > 2) {{
                            score = 70;
                        }}
                        // Word overlap
                        else {{
                            ...
                            const commonWords = answerWords.filter(w => optWords.includes(w));
                            score = commonWords.length * 20;
                        }}
                        
                        // Boost for common affirmative answers
                        if (['yes', 'true', 'agree', 'accept'].includes(optText)) {{
                            score += 5;
                        }}
                ...
                // Check if answer indicates affirmative/yes
                const isAffirmative = (answer) => {{
                    if (!answer) return false;
                    const affirmativeWords = ['yes', 'true', 'agree', 'accept', 'willing', 'available', 'ready', 'comfortable'];
                    return affirmativeWords.some(word => answer.toLowerCase().includes(word));
                }};

                // Check if answer indicates negative/no
                const isNegative = (answer) => {{
                    if (!answer) return false;
                    const negativeWords = ['no', 'false', 'not', 'cannot', 'unwilling'];
                ...
                if (isLinkedIn) {{
                    ...
                    if (successModal) {{
                        ...
                        if (modalText.includes('Application sent') || modalText.includes('Application submitted')) {{
                            ...
                            if (doneBtn && doneBtn.innerText.toLowerCase().includes('done')) {{
                                doneBtn.click();
                                // Wait for modal to fully dismiss and DOM to settle
                    ...
                    if (successVisible || successInText) {{
                        ...
                        // Small wait to allow DOM to settle after dismissal
                        ...
                        for (const group of formGroups) {{
                            ...
                            if (textInput && !textInput.disabled && textInput.offsetParent !== null) {{
                                ...
                                if (!textInput.value || textInput.value.trim() === '') {{
                                    ...
                                    let value = answer || (isNumericField ? '4' : '');
                            ...
                            if (select && select.offsetParent !== null) {{
                                ...
                                if (select.selectedIndex <= 0) {{
                                    ...
                                    if (answer) {{
                                        const bestMatch = findBestOptionMatch(answer, selectOptions);
                                        if (bestMatch) {{
                            ...
                            if (radios.length > 0) {{
                                ...
                                if (!clicked && answer) {{
                                    ...
                                    const bestMatch = findBestOptionMatch(answer, radioOptions);
                                    if (bestMatch) {{
                            ...
                            if (checkboxes.length > 0) {{
                                ...
                                options = Array.from(checkboxes).map(cb => {{
                                    const label = cb.parentElement?.innerText || cb.nextElementSibling?.innerText || cb.value || '';
                                    return label.trim();
                                }}).filter(t => t);
                                
                                // If answer indicates affirmative, try to check relevant boxes
                                if (isAffirmative(answer)) {{
                                    for (const checkbox of checkboxes) {{
                                        if (!checkbox.checked) {{
                                            const label = checkbox.parentElement?.innerText || checkbox.nextElementSibling?.innerText || '';
                                            // Check if answer matches this checkbox label
                                            const labelLower = label.toLowerCase();
                                            const answerLower = answer.toLowerCase();
                                            
                                            if (labelLower.includes(answerLower) || 
                                                answerLower.includes(labelLower) ||
                                                ['yes', 'agree', 'accept', 'true'].some(w => labelLower.includes(w))) {{
                                                checkbox.click();
                                                selectedOption = (selectedOption ? selectedOption + ', ' : '') + label.trim();
                                                finalAnswer = selectedOption || 'Checked';
                                            }}
                                        }}
                                    }}
                                }}
                                ...
                                if (checkedBoxes.length > 0 && !selectedOption) {{
                                    selectedOption = checkedBoxes.map(cb => 
                                        cb.parentElement?.innerText || cb.nextElementSibling?.innerText || cb.value || 'Checked'
                                    ).join(', ');
                                    finalAnswer = selectedOption;
                                }}
                            }}
